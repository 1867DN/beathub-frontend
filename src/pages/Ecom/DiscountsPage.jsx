...
export default function DiscountsPage() {
  ...
  // ── Pre-filter: only discounted products ──
  const discountedProducts = useMemo(
    () => allProducts.filter(p => (p.discount_percent || 0) > 0),
    [allProducts]
  )

  const maxPrice = useMemo(() => {
    if (!discountedProducts.length) return 0
    return Math.max(...discountedProducts.map(p => p.price_list || p.price || 0))
  }, [discountedProducts])

  const brandCount = useMemo(() => {
    const map = {}
    discountedProducts.forEach(p => { if (p.brand_id) map[p.brand_id] = (map[p.brand_id] || 0) + 1 })
    return map
  }, [discountedProducts])

  const catCount = useMemo(() => {
    const map = {}
    discountedProducts.forEach(p => { if (p.category_id) map[p.category_id] = (map[p.category_id] || 0) + 1 })
    return map
  }, [discountedProducts])

  const displayedProducts = useMemo(() => {
    let result = [...discountedProducts]
    if (filterBrands.size > 0) result = result.filter(p => filterBrands.has(p.brand_id))
    if (filterCategories.size > 0) result = result.filter(p => filterCategories.has(p.category_id))
    if (appliedPriceMin != null) result = result.filter(p => (p.price || 0) >= appliedPriceMin)
    if (appliedPriceMax != null) result = result.filter(p => (p.price || 0) <= appliedPriceMax)
    switch (sortBy) {
      case 'alpha_asc': result.sort((a, b) => a.name.localeCompare(b.name, 'es')); break
      case 'alpha_desc': result.sort((a, b) => b.name.localeCompare(a.name, 'es')); break
      case 'price_asc': result.sort((a, b) => (a.price || 0) - (b.price || 0)); break
      case 'price_desc': result.sort((a, b) => (b.price || 0) - (a.price || 0)); break
    }
    return result
  }, [discountedProducts, filterBrands, filterCategories, appliedPriceMin, appliedPriceMax, sortBy])

  const toggleBrandFilter = (id) => setFilterBrands(prev => { const n = new Set(prev); n.has(id) ? n.delete(id) : n.add(id); return n })
  const toggleCategoryFilter = (id) => setFilterCategories(prev => { const n = new Set(prev); n.has(id) ? n.delete(id) : n.add(id); return n })
  ...
  const activeFilterCount = filterBrands.size + filterCategories.size + (appliedPriceMin != null ? 1 : 0) + (appliedPriceMax != null ? 1 : 0)
  const sortLabel = { alpha_asc: 'A → Z', alpha_desc: 'Z → A', price_asc: 'Menor precio', price_desc: 'Mayor precio' }
  const visibleBrands = showAllBrands ? brands.filter(b => (brandCount[b.id_key] || 0) > 0) : brands.filter(b => (brandCount[b.id_key] || 0) > 0).slice(0, 8)
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      <nav className="flex items-center gap-1.5 text-sm text-gray-500 mb-4 flex-wrap">
        ...
        <span className="text-gray-800 font-medium">Descuentos</span>
      </nav>
      ...
      <div className="flex items-end justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">Productos con descuento activo</p>
        </div>
        <button
      ...
      {activeFilterCount > 0 && (
        <div className="flex flex-wrap gap-2 mb-5">
          {[...filterBrands].map(id => {
            ...
            return b ? (
              <span key={id} className="flex items-center gap-1 bg-gray-100 text-xs font-semibold px-3 py-1 rounded-full">
                {b.name}<button onClick={() => toggleBrandFilter(id)} className="ml-0.5 hover:text-red-500">×</button>
              </span>
            ) : null
          })}
          {[...filterCategories].map(id => {
            ...
            return c ? (
              <span key={id} className="flex items-center gap-1 bg-gray-100 text-xs font-semibold px-3 py-1 rounded-full">
                {c.name}<button onClick={() => toggleCategoryFilter(id)} className="ml-0.5 hover:text-red-500">×</button>
              </span>
            ) : null
          })}
      ...
      {discountedProducts.length === 0 ? (
        <div className="text-center py-20">
          ...
          <p className="text-gray-500 text-lg">No hay productos con descuento por el momento.</p>
          <p className="text-gray-400 text-sm mt-1">Volvé pronto para ver las ofertas.</p>
          <Link to="/productos" className="mt-4 inline-block text-primary font-semibold hover:underline">Ver todos los productos</Link>
        </div>
          ...
          {displayedProducts.map(product => (
            ...
            >
              <div className="relative bg-gradient-to-br from-gray-100 to-gray-200 aspect-square flex items-center justify-center overflow-hidden">
                ...
                <span className="absolute top-2 left-2 bg-red-500 text-white text-xs font-black px-2 py-1 rounded-lg shadow">
                  -{product.discount_percent}%
                </span>
                {product.is_new === 'true' && (
                  <span className="absolute top-2 right-2 bg-purple-600 text-white text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wide">
                    Nuevo
                  </span>
                )}
              </div>
              <div className="p-3">
                {product.brand_id && brandById[product.brand_id] && (
                  <span className="text-xs text-gray-400 font-medium uppercase tracking-wide">{brandById[product.brand_id].name}</span>
                )}
      ...
      >
        <div className="flex-1 overflow-y-auto">
          <div className="flex justify-end px-5 py-4 border-b border-gray-100">
            <button onClick={() => setPanelOpen(false)} className="text-2xl font-light text-gray-400 hover:text-gray-700 transition leading-none">×</button>
          </div>
              ...
              <div className="px-5 pb-3 space-y-2 bg-gray-50">
                {[{ value: 'alpha_asc', label: 'A → Z' }, { value: 'alpha_desc', label: 'Z → A' }, { value: 'price_asc', label: 'Menor precio primero' }, { value: 'price_desc', label: 'Mayor precio primero' }].map(opt => (
                  <label key={opt.value} className="flex items-center gap-3 cursor-pointer py-1.5">
                    <input type="radio" name="discSort" checked={sortBy === opt.value} onChange={() => setSortBy(opt.value)} className="accent-black" />
          ...
          <div className="border-b border-gray-100">
            ...
            {openSection === 'categories' && (
              <div className="px-5 pb-3 space-y-2 bg-gray-50">
                {categories.filter(c => (catCount[c.id_key] || 0) > 0).map(cat => (
                  <label key={cat.id_key} className="flex items-center gap-3 cursor-pointer py-1.5">
                    <input type="checkbox" checked={filterCategories.has(cat.id_key)} onChange={() => toggleCategoryFilter(cat.id_key)} className="accent-black rounded" />
                    <span className="text-sm font-semibold text-gray-700">{cat.name} <span className="text-gray-400 font-normal">({catCount[cat.id_key]})</span></span>
                  </label>
                ))}
              </div>
            )}
          ...
          {/* Marca */}
          <div className="border-b border-gray-100">
            <button className="w-full flex items-center justify-between px-5 py-2.5 hover:bg-primary/10 hover:text-primary transition" onClick={() => toggleSection('brands')}>
              <span className="text-sm font-semibold text-gray-700 uppercase tracking-widest">Marca</span>
              <span className="text-gray-400">›</span>
            </button>
            {openSection === 'brands' && (
              <div className="px-5 pb-3 space-y-2 bg-gray-50">
                {visibleBrands.map(brand => (
                  <label key={brand.id_key} className="flex items-center gap-3 cursor-pointer py-1.5">
                    <input type="checkbox" checked={filterBrands.has(brand.id_key)} onChange={() => toggleBrandFilter(brand.id_key)} className="accent-black rounded" />
                    <span className="text-sm font-semibold text-gray-700">{brand.name} <span className="text-gray-400 font-normal">({brandCount[brand.id_key] || 0})</span></span>
                  </label>
                ))}
                {brands.filter(b => (brandCount[b.id_key] || 0) > 0).length > 8 && (
                  <button onClick={() => setShowAllBrands(!showAllBrands)} className="text-xs text-primary font-semibold hover:underline mt-1">
                    {showAllBrands ? 'Ver menos' : `Ver ${brands.filter(b => (brandCount[b.id_key] || 0) > 0).length - 8} más`}
                  </button>
                )}
              </div>
            )}
