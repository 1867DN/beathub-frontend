...
export default function BrandSliderReverse() {
  ...
  useEffect(() => {
    brandsAPI.getAll()
      ...
      .catch(err => console.error('Error fetching brands (reverse):', err))
  ...
  useEffect(() => {
    ...
    const SPEED = 0.5 // same speed as BrandSlider

    const tick = () => {
      if (!pausedRef.current) {
        const singleWidth = track.scrollWidth / 3

        // Initialize starting position on first tick
        ...
        // When we've scrolled back to 0, jump back by one singleWidth — seamless
        if (posRef.current >= 0) {
          posRef.current -= singleWidth
        }
    ...
    >
      ...
      >
        {tripled.map((brand, idx) => (
          <BrandLogo key={`rev-${brand.id_key}-${idx}`} brand={brand} />
        ))}
      </div>
    </div>
  )
